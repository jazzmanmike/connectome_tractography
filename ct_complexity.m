function [ RDN, DGN, CMP ] = ct_complexity( network )
%CT_COMPLEXITY    computes redundancy & degeneracy
%
%   [RDN, DGN, CMP] = ct_complexity(network); 
%
...
for i = 1:N; % for each parcel in turn
    ...
    for j = 1:N; % work through all other parcels in turn
        ...
        RDN(i,j) = ct_mutual_information(grotI, grotJ); %gives a scalar value
    end
end
...
for i = 1:N; % for each parcel in turn
    ...
    for j = 1:N; % work through all other parcels in turn
        ...
        DGN(i,j) = ct_mutual_information(grotI, grotJ); %gives a scalar value
    end
end

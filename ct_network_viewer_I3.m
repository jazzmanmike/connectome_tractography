function ct_network_viewer_I3( nodes, network, xyz, target, tumour )
%CT_NETWORK_VIEWER_I3 Plots 3-plane views of network for Icube interface
%
%   cs_network_viewer_I3(nodes, network, xyz, target);
%
%   Inputs: nodes,      sizes for nodes (e.g. hub status)
%           network,    corresponding network matrix
%           xyz,        Euclidean co-ordinates  
%           target,     node to view
%           tumour,     nodes overlapping the tumour
%
...
%
% Michael Hart, University of British Columbia, January 2021
...
%[C, q] = core_periphery_dir(network, gamma);
%[Erange, eta, Eshort, fs] = erange(network);
%T = path_transitivity(network, 'inv');

%% Define node colours

Colours = zeros(nNodes, 1);
Colours(:, 1) = 'w'; %everything in silver
Colours(tumour, 1) = 'r'; %tumour in red
%Colours(target, 1) = 'y'; %target node in yellow - could be done later
